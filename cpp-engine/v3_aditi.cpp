...
public:
    ...
    //     bool flag = false;
    //     int maxTime = 10000;
    //     auto move= alphaBetaPruning(maxDepth, -INFINITY, , start, flag, maxTime);
    //     return move.second;
    // }
    ...
    pair<float, string> alphaBetaPruning(int depth, float alpha, float beta, std::chrono::time_point<std::chrono::high_resolution_clock> start, bool &flag, double maxTime = 10000) {

        if (flag) {
            return {0, ""};
        }
        if (chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count() > maxTime) {
            flag = true;
            return {0, ""};
        }
        ...
        for (const auto& move : moves) {
            ...
            if (transpositionTable.find(board.hash()) != transpositionTable.end()) {
                ...
                if (transposition.second >= depth) {
                    ...
                    score = alphaBetaPruning(depth - 1, alpha, beta, start, flag, maxTime).first;
                }
            } else {
                score = alphaBetaPruning(depth - 1, alpha, beta, start, flag, maxTime).first;
            }
    ...
    string iterativeDeepening(int maxTime = 10000) {
        std::chrono::time_point<std::chrono::high_resolution_clock> start = chrono::high_resolution_clock::now();
        auto end = std::chrono::high_resolution_clock::now();
        int depth = 1;
        bool flag = false;
        string bestMove;
        pair<float, string> move;
        while (chrono::duration_cast<std::chrono::milliseconds>(end - start).count() < maxTime) {
            move = alphaBetaPruning(depth, -10000000, 10000000, start, flag, maxTime);
            depth++;
            auto end = std::chrono::high_resolution_clock::now();
            if (flag) {
                break;
...
int main() {
    ...
    while (std::getline(std::cin, command)) {
        if (command == "uci") {
            ...
            auto move = engine.iterativeDeepening(maxTimeInMillis);
            ...
            auto move = engine.iterativeDeepening(10000);
            float eval = engine.evaluate();
            // std::cout << "info depth " << move.second << " score cp " << static_cast<int>(eval * 100) << std::endl;
            sendResponse("bestmove " + move);
